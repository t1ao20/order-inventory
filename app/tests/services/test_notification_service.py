...
def make_order(order_id: UUID) -> Order:
    return Order(
        id=order_id,
        employee_id=1,
        vendor_user_id=37,
...
def test_notify_order_cancelled_posts_detailed_payload(monkeypatch):
    ...
    fake_repo = FakeOrderRepository(make_order(order_id))
    ...
    assert payload["title"] == "您的訂單已取消：招牌烤雞"
    ...
    assert "單價：109" in payload["content"]
    assert "總金額：218" in payload["content"]
    assert "取餐日期：2026-06-10" in payload["content"]
...
def test_notify_order_cancelled_falls_back_when_order_cannot_be_loaded(monkeypatch):
    ...
    assert payload["user_id"] == 1
    assert payload["title"] == "通知訂單 abc-order 取消"
