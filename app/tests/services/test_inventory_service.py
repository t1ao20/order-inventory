...
def test_set_inventory_cancels_latest_orders_when_lowering_max(monkeypatch):
    ...
    monkeypatch.setattr(inventory_service, "notify_order_cancelled", lambda order_id, user_id, cancel_reason=None: asyncio.sleep(0, result=notify_calls.append((order_id, user_id, cancel_reason))))
    ...
    assert notify_calls == [
        ("00000000-0000-4000-8000-000000000103", 1, "庫存調整，餐點數量不足"),
        ("00000000-0000-4000-8000-000000000203", 1, "庫存調整，餐點數量不足"),
        ("00000000-0000-4000-8000-000000000303", 1, "庫存調整，餐點數量不足"),
    ]
